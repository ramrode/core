...
services:
  ella-core:
    image: ghcr.io/ellanetworks/ella-core:v1.17.0
    configs:
      - source: ella_config
        target: /core.yaml
...
This example uses `tcx` mode, which works on the veth interfaces containers get. On a host whose driver supports it, use `xdp-native` mode for the highest performance — see [Config file](https://docs.ellanetworks.com/reference/config_file/index.md).
...
```
kubectl apply -k github.com/ellanetworks/core/k8s?ref=v1.17.0 -n ella
```
