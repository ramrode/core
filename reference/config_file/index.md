...
- `datapath` (object, optional): The datapath configuration. When omitted, the datapath attaches at the XDP hook in driver mode where the network interface supports it, and at the TCX hook otherwise.
  - `attach-mode` (string, optional): The kernel hook the datapath attaches to: `xdp-native`, `tcx`, or `xdp-generic`. See [the eBPF attach mode explanation](https://docs.ellanetworks.com/explanation/user_plane_packet_processing_with_ebpf/index.md).
